import findIndexes, { titleRegex } from "./findIndexes";

export default function parseTraitActions(arr: string[]) {
  const actionIndexes = findIndexes(arr);
  if (!actionIndexes.Actions && !actionIndexes.ACTIONS)
  ...
  actions
    .at(-1)
    ?.content.push({ name: currentName, description: currentDescription });
  return { traits, actions };
}
